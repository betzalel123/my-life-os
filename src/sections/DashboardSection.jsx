...
export default function DashboardSection({
  ...
  const dopamineMenu = [
    ...
    { title: 'מתיחה קטנה', desc: 'לשחרר כתפיים וצוואר לשתי דקות' },
  ];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-top-4 duration-700">
      <div className="lg:col-span-2 space-y-8">
        <section className="text-white shadow-2xl relative overflow-hidden transition-all duration-700 ease-in-out bg-slate-900 p-4 rounded-2xl opacity-80 h-20">
          <div className="relative z-10 flex items-center justify-between h-full px-4">
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center shadow-lg animate-pulse bg-indigo-600">
            ...
            <div className="text-white/30 font-mono text-xl">{formatTime(timeLeft)}</div>
          </div>
        </section>
            ...
            {visibleTasks.map((task) => (
              ...
              >
                ...
                {task.subTasks && task.subTasks.length > 0 && (
                  <div className="mt-3 mr-8 space-y-1.5 border-r-2 border-indigo-100/50 pr-4 animate-in slide-in-from-top-1 duration-300">
                      ...
                      >
                        ...
                        <span
                          className={`text-[13px] font-medium transition-all ${
                            st.completed ? 'line-through text-slate-300' : 'text-slate-600'
                          }`}
                        >
                          {st.text}
                        </span>
      ...
      <div className="space-y-8">
        <section className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200/50 relative overflow-hidden">
          ...
          <button className="w-full mt-4 py-2 bg-slate-50 text-slate-500 rounded-xl text-[10px] font-black hover:bg-slate-100 transition-all uppercase tracking-widest">
            לניהול מלא
          </button>
        </section>
