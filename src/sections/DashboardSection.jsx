...
export default function DashboardSection({
  ...
  const [dopamineOptions, setDopamineOptions] = useState([
    {
      ...
      desc: 'בחרי שיר שאת אוהבת ופשוט תזיזי את הגוף למשך 3 דקות.',
      iconName: 'Music',
      accent: 'text-rose-400'
    },
    {
      title: 'סידור מיקרו של משטח קטן',
      desc: 'פני רק פינה קטנה אחת — שידה או חלק קטן מהשולחן.',
      iconName: 'Gift',
      accent: 'text-indigo-400'
    },
    {
      title: 'שרבוט יצירתי ל־5 דקות',
      desc: 'קחי דף ועט ותעשי קווים בלי לחשוב יותר מדי.',
      iconName: 'Lightbulb',
  ...
  const handleSmartAddTask = async (e) => {
    ...
    const energyPrompt = `
אתה ממיין משימות לפי רמת אנרגיה.

משימה:
"${newTask}"

כללים:
- high = משימה שדורשת ריכוז גבוה, חשיבה עמוקה, למידה קשה, פתרון בעיות, כתיבה משמעותית, שיחה מורכבת, אימון מאתגר.
- medium = משימה רגילה של תפקוד, סידורים, מיילים, ניקיון רגיל, קניות, בישול, טלפונים.
- low = משימה קטנה, טכנית, קצרה, כמעט אוטומטית, שאפשר לעשות גם כשעייפים.

אל תחזיר תמיד low.
אם המשימה דורשת מאמץ מנטלי אמיתי, אל תבחר low.
...
`;
    ...
    try {
      ...
      const detectedEnergy = ['high', 'medium', 'low'].includes(data?.energy)
        ? data.energy
        : 'medium';
...
חוקים:
- כל הרעיונות חייבים להיות שונים אחד מהשני.
- לכל רעיון תבחר אייקון מתאים מתוך הרשימה בלבד:
Music, Coffee, Flame, Smile, Zap, Lightbulb, Gift, Sparkles
- אל תשתמש באותו iconName לכל שלושת הרעיונות אלא אם ממש אין ברירה.
- לכל רעיון תבחר accent מתוך הרשימה בלבד:
...
`;
    ...
    try {
      ...
      const data = await response.json();
      console.log('dopamine response:', data);

      if (data?.options && Array.isArray(data.options)) {
        setDopamineOptions(data.options);
      } else {
        console.error('Invalid dopamine response:', data);
      }
    } catch (error) {
          ...
          <div className="flex items-center justify-between mb-6">
            <button
              ...
              className={`px-4 py-2 rounded-xl bg-rose-50 text-rose-500 font-black text-sm flex items-center gap-2 transition-all ${
                isLoadingGemini ? 'opacity-50 cursor-not-allowed' : 'hover:bg-rose-100 shadow-sm'
              }`}
            >
              {isLoadingGemini ? (
          ...
          {filteredTasks.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredTasks.map((task) => (
                <div
                  key={task.id}
                  className="rounded-[1.7rem] border border-slate-200 bg-slate-50 px-6 py-5 flex items-center gap-4 hover:shadow-sm transition-all cursor-pointer"
                >
